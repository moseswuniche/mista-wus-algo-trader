...
def load_secrets_from_aws(secret_name: str, region_name: str) -> Optional[Dict[str, str]]:
    ...
    try:
        ...
        logger.info(f"Attempting to load secrets from AWS Secrets Manager: {secret_name} in {region_name}")
        ...
        logger.error("AWS credentials not found. Ensure credentials are configured (e.g., environment variables, instance profile).", exc_info=True)
        return None
    except ClientError as e:
        logger.error(f"AWS ClientError loading secret '{secret_name}': {e}", exc_info=True)
        ...
        logger.error(f"An unexpected error occurred loading secrets from AWS: {e}", exc_info=True)
        return None
