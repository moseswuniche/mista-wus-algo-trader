...
# Constants
DEFAULT_SYMBOLS = [
    "BTCUSDT", "XRPUSDT", "ALGOUSDT", "ETHUSDT", 
    "HBARUSDT", "ADAUSDT", "SOLUSDT", "LTCUSDT", "SUIUSDT"
]
...
def fetch_historical_klines(client: Client, symbol: str, interval: str, start_str: str, end_str: str = None) -> List:
    ...
    while True:
        try:
            logger.debug(f"Fetching chunk starting from {datetime.fromtimestamp(start_dt_ms / 1000)}...")
            ...
            logger.error(f"Binance API Error for {symbol}: Code={bae.code}, Message={bae.message}")
            ...
            logger.error(f"An unexpected error occurred fetching data for {symbol}: {e}", exc_info=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-i", "--interval", type=str, default=DEFAULT_INTERVAL,
